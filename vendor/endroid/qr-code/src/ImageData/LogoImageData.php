...
final class LogoImageData
{
    ...
    public static function createForLogo(LogoInterface $logo): self
    {
        ...
        if (!self::isValidPath($path)) {
            throw new \InvalidArgumentException(sprintf('Invalid logo path "%s"', $path));
        }
        ...
        try {
            ...
            if (!is_string($data)) {
                ...
                throw new \Exception(sprintf('Could not read logo image data from path "%s": %s', $path, $errorDetails));
            }

            if (false !== filter_var($path, FILTER_VALIDATE_URL)) {
                $mimeType = self::detectMimeTypeFromUrl($path);
            } else {
                $mimeType = self::detectMimeTypeFromPath($path);
            }
            ...
            if (!$image) {
                $errorDetails = error_get_last()['message'] ?? 'invalid data';
                throw new \Exception(sprintf('Unable to parse image data at path "%s": %s', $path, $errorDetails));
            }
            ...
            throw new LogoImageDataException(sprintf("Error processing image data from path '%s': %s", $path, $e->getMessage()), 0, $e);
        }
    }
    ...
    private static function detectMimeTypeFromUrl(string $url): string
    {
        ...
        $contentType = is_array($headers['content-type']) ? $headers['content-type'][1] : $headers['content-type'];

        if (is_string($contentType) && !empty($contentType)) {
            return $contentType;
        }

        throw new \Exception(sprintf('Could not retrieve content type for logo URL "%s"', $url));
    }
...
class LogoImageDataException extends \Exception {}
