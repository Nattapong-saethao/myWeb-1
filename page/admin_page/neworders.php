...
<body>
    ...
    <div class="content">
        ...
        <table class="table table-bordered">
            ...
            <tbody>
                <?php
                // SQL query สำหรับดึงข้อมูล orders ที่มีสถานะ "pending" พร้อมข้อมูลสมาชิกและสินค้า
                $sql = "SELECT
                            o.order_id,
                        ...
                        WHERE
                            o.status = 'pending' AND od.selected = 1
                        GROUP BY
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo "<td>
                            <a href='#' class='btn btn-primary btn-warning' onclick='goToDetail(\"" . $row["order_id"] . "\")'>ตรวจสอบ</a>
                            <a href='#' class='btn btn-danger btn-sm' onclick='deleteOrder(\"" . $row["order_id"] . "\")'>ลบ</a>
                        </td>";
                        echo "</tr>";
                    }
                } else {
                    echo "<tr><td colspan='7'>ไม่พบคำสั่งซื้อที่อยู่ในสถานะ รอดำเนินการ หรือไม่มีสินค้าที่ถูกเลือก</td></tr>";
                }
                ?>
            </tbody>
        </table>
    ...
    <script>
        function goToDetail(orderId) {
            ...
            window.location.href = 'checkorder.php?id=' + orderId + '&status=' + 'pending';
        }
