                        ...
                        <tr>
                            ...
                            <td>
                                <?php if ($product['image']): ?>
                                    <img src="<?php echo htmlspecialchars($product['image']); ?>" alt="Product Image" class="img-fluid" style="max-width: 100px; max-height: 100px;">
                                <?php else: ?>
                                    No image
                                <?php endif; ?>
                            </td>
                            <td>
                                ...
                                <form method="POST" class="deleteProductForm" data-product-id="<?php echo $product['product_id']; ?>" style=" display:inline;">
                                    <button type="submit" class="btn btn-sm btn-danger deleteProductBtn">ลบ</button>
                                </form>
                            </td>
                        </tr>
                    <?php endforeach; ?>
...
<script>
    ...
    $(document).ready(function() {
        $('.deleteProductForm').on('submit', function(e) {
            ...
            var productId = $(this).data('product-id'); // ดึง productId จาก attribute data
            var form = $(this);


            if (confirm("คุณต้องการลบสินค้านี้ใช่หรือไม่?")) {
                $.ajax({
                    url: 'delete_product.php', // URL ของไฟล์ PHP ที่ใช้ลบสินค้า
                    type: 'POST',
                    data: {
                        product_id: productId
                    },
                    success: function(response) {
                        console.log("Response:", response); // Log response for debug
                    ...
                    error: function() {
                        $('#resultMessage').text('เกิดข้อผิดพลาดในการลบสินค้า');
                        $('#resultModal').modal('show');
                    }
                });
            }
