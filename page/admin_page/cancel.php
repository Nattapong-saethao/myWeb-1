...
<body>
    ...
    ?>
    <div class="content">
        <h2>จัดการคำสั่งซื้อที่ถูกยกเลิก</h2>
        <table class="table table-bordered">
            ...
            <tbody>
                <?php
                // SQL query สำหรับดึงข้อมูล orders ที่มีสถานะ "cancel" พร้อมข้อมูลสมาชิกและสินค้า
                $sql = "SELECT
                            o.order_id,
                        ...
                        WHERE
                            o.status = 'cancel' AND od.selected = 1
                        GROUP BY
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo "<td>
                            <a href='#' class='btn btn-primary ' onclick='goToDetail5(\"" . $row["order_id"] . "\")'>รายละเอียด</a>
                            <a href='#' class='btn btn-danger btn-sm' onclick='deleteOrder(\"" . $row["order_id"] . "\")'>ลบ</a>
                        </td>";
                        echo "</tr>";
                    }
                } else {
                    echo "<tr><td colspan='7'>ไม่พบคำสั่งซื้อที่อยู่ในสถานะ ยกเลิก หรือไม่มีสินค้าที่ถูกเลือก</td></tr>";
                }
                ?>
            </tbody>
        </table>
    ...
    <script>
        function goToDetail5(orderId) {
            ...
            window.location.href = 'checkorder_cancel.php?id=' + orderId + '&status=' + 'cancel';
        }
